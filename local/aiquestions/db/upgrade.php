...
function xmldb_local_aiquestions_upgrade($oldversion) {
    ...
    if ($oldversion < 2024071503) {
        // Define table quiz_evaluation to be created.
        $table = new xmldb_table('quiz_evaluation');
        ...
        // Adding keys to table quiz_evaluation.
        $table->add_key('primary', XMLDB_KEY_PRIMARY, array('id'));
    
        // Conditionally launch create table for quiz_evaluation.
        if (!$dbman->table_exists($table)) {
            $dbman->create_table($table);
        }
        upgrade_plugin_savepoint(true, 2024071503, 'local', 'aiquestions');
    }
